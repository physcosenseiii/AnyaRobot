    ...
    if match := re.match(r"rm_chat\((.+?)\)", query.data):
        ...
        if is_kuki := sql.rem_kuki(chat.id):
            ...
            update.effective_message.edit_text(
                f"Anya Chatbot disabled by {mention_html(user.id, user.first_name)}.",
...
def kukiadd(update: Update, context: CallbackContext) -> str:
    ...
    if match := re.match(r"add_chat\((.+?)\)", query.data):
        ...
        if is_kuki := sql.set_kuki(chat.id):
            ...
            update.effective_message.edit_text(
                f"Anya Darling Chatbot enabled by {mention_html(user.id, user.first_name)}.",
                parse_mode=ParseMode.HTML,
            )
...
def chatbot(update: Update, context: CallbackContext):
    ...
    if message.text and not message.document:
        ...
        # )
        kukiurl = requests.get(f"https://api.safone.me/chatbot?query={Message}&user_id=1234567&bot_name=Anya&bot_master=chirag")
